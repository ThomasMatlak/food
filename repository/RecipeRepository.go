...
func GetRecipe(id string) (*model.Recipe, error) {
	...
	return neo4j.ExecuteWrite(ctx, session, func(tx neo4j.ManagedTransaction) (*model.Recipe, error) {
		...
		*created = rawCreated.Time()
		...
		node.GetProperties()
		rawLastUpdated, err := neo4j.GetProperty[neo4j.LocalDateTime](node, "lastUpdated")
		lastUpdated := new(time.Time)
		if err != nil {
			lastUpdated = nil
		} else {
			*lastUpdated = rawLastUpdated.Time()
		}

		return &model.Recipe{Id: id, Title: title, Steps: steps, Created: created, LastModified: lastUpdated}, nil
	})
}
